 ...
 *
 * Based on the SMAZ algorithm by Salvatore Sanfilippo (antirez),
 * originally licensed under BSD. Adapted for C++ / Haiku.
...
static const char* kSmazRcb[254] = {
...
// Prefix used to identify SMAZ-compressed messages (meshcore-open compatible)
static const char kSmazPrefix[] = "s:";
static const size_t kSmazPrefixLen = 2;


inline bool
SmazIsCompressed(const char* text)
{
	return text != NULL && text[0] == 's' && text[1] == ':';
}


inline int
SmazCompress(const char* in, int inlen, char* out, int outlen)
{
	...
	while (inlen > 0) {
		...
		if (inlen > 1)
			h2 += (unsigned char)in[1];
		if (inlen > 2)
			h3 = h2 ^ (unsigned char)in[2];
		if (j > inlen)
			j = inlen;

		for (; j > 0; j--) {
			switch (j) {
				case 1: slot = kSmazCb[h1 % 241]; break;
				case 2: slot = kSmazCb[h2 % 241]; break;
				default: slot = kSmazCb[h3 % 241]; break;
			}
			while (slot[0] != '\0') {
				if ((unsigned char)slot[0] == j
					&& memcmp(slot + 1, in, j) == 0) {
					if (verblen > 0) {
...
inline int
SmazDecompress(const char* in, int inlen, char* out, int outlen)
{
